...
last_error_id, last_ann_id, last_delete_id = 0, 0, 0
...
def form_new_lines(line1, line2, errors_with_tags, sentences):
    global last_error_id, last_ann_id, last_delete_id, used_error_ids
    global text
                ...
                for tag_id in token.keys():
                    ...
                    if not (error_idx_1 == 0 and error_idx_2 == 0):
                        ...
                        if ann_tag == 'Redundant_comp':
                            ...
                            new_ann_id = 'A' + str(last_delete_id) if ann_id[0] != 'A' else ann_id

                            new_tag_parts = ' '.join([ann_tag, str(error_idx_1), str(error_idx_2)])
                            new_ann_notes = 'Delete ' + str(new_error_id)

                            new_line_1 = '\t'.join([new_error_id, new_tag_parts, error]) + '\n'
                            new_line_2 = '\t'.join([new_ann_id, new_ann_notes]) + '\n'
                        else:
                            new_error_id = 'T' + str(last_error_id) if new_error else error_id
...
def process_files():
    global file_num_total, file_num, last_error_id, last_ann_id, last_delete_id
    global text
